...
function renderPenKPI(r) {
  ...
  document.getElementById('penKpiCards').innerHTML = `
    <div class="mcard"> <div class="ml">Pensione INPS netta</div> <div class="mv" style="color:var(--acc-inps)">${fmt(pensioneNettaMens)}<span style="font-size:11px;opacity:.6">/m</span></div> <div class="ms">≈ ${fmt(inpsReal)}/m in € di oggi · coeff. ${(coeffTrasf*100).toFixed(3)}%${r.coeffTrasfBase && Math.abs(r.coeffTrasfBase-coeffTrasf)>1e-5?` (2025: ${(r.coeffTrasfBase*100).toFixed(3)}%)`:''}</div> </div> <div class="mcard"> <div class="ml">Rendita Fondo Pensione</div> <div class="mv" style="color:var(--acc-fp)">${fmt(rendFPMens)}<span style="font-size:11px;opacity:.6">/m</span></div> <div class="ms">≈ ${fmt(fpReal)}/m in € di oggi · cap. ${fmt(capFP)}</div> </div> ${tfrInfo && tfrInfo.fondo.lordo > 0 ? `
    <div class="mcard"> <div class="ml">TFR nel fondo (quota)</div> <div class="mv" style="color:var(--acc-tfr)">${fmt(tfrInfo.fondo.netto)}</div> <div class="ms">${fmt(tfrInfo.fondo.lordo)} lordo → ${fmt(tfrInfo.fondo.netto)} netto (tass. agevolata ${(tfrInfo.fondo.aliq*100).toFixed(1)}%) · già incluso nel capitale FP</div> </div>` : ''}
    <div class="mcard"> <div class="ml">Prelievo ETF Portfolio</div> <div class="mv" style="color:var(--acc-etf)">${fmt(etfPrelievoMens)}<span style="font-size:11px;opacity:.6">/m</span></div> <div class="ms">≈ ${fmt(etfReal)}/m in € di oggi · cap. ${fmt(etfCap)} · SWR 4%</div> </div> ${capTfrAzNetto > 0 ? `
...
function renderPenINPS(r) {
  ...
  document.getElementById('penINPSDetail').innerHTML = `
    <div style="display:flex;gap:10px;flex-wrap:wrap;margin-bottom:14px"> <div class="mcard"><div class="ml">Montante al pensionamento</div><div class="mv" style="color:var(--data)">${fmt(cumMontante)}</div><div class="ms">Rivalutato PIL nom. (${((penState.pil+penState.infl)*100).toFixed(1)}%/a)</div></div> <div class="mcard"><div class="ml">Coeff. trasformazione</div><div class="mv" style="color:var(--data)">${(coeffTrasf*100).toFixed(3)}%</div><div class="ms">Età ${penState.retAge}${r.coeffTrasfBase && Math.abs(r.coeffTrasfBase-coeffTrasf)>1e-5?` · futuro stimato (2025: ${(r.coeffTrasfBase*100).toFixed(3)}%)`:'· tabella INPS 2025'}</div></div> <div class="mcard"><div class="ml">Pensione lorda annua</div><div class="mv" style="color:var(--data)">${fmt(pensioneLordaAnn)}</div><div class="ms">${fmt(pensioneLordaMens)}/mese</div></div> <div class="mcard"><div class="ml">IRPEF stimata annua</div><div class="mv" style="color:var(--red)">${fmt(irpefAnn)}</div><div class="ms">Scaglioni 2025 + detrazione</div></div> <div class="mcard"><div class="ml">Pensione netta mensile</div><div class="mv" style="color:var(--data)">${fmt(pensioneNettaMens)}</div><div class="ms">× 13 mensilità INPS</div></div> <div class="mcard"><div class="ml">Tasso di sostituzione</div><div class="mv" style="color:${tsCol}">${(tassoSost*100).toFixed(1)}%</div><div class="ms">Lorda / RAL finale</div></div> </div> <div style="background:#f4f5f7;border:1px solid #d9d9d9;border-radius:var(--radius-sm);padding:12px 16px;font-size:12px;color:#374151;line-height:1.7"> <strong>Formula (metodo contributivo):</strong>Pensione lorda = Montante (${fmt(cumMontante)}) × Coefficiente (${(coeffTrasf*100).toFixed(3)}%) = <strong>${fmt(r.pensioneLordaAnn)}/anno</strong>.<br>Montante rivalutato a PIL nom. (PIL reale ${(penState.pil*100).toFixed(1)}% + inflaz. ${(penState.infl*100).toFixed(1)}% = ${((penState.pil+penState.infl)*100).toFixed(1)}%/a).
    </div>`;
}
...
function renderPenAccTable(r) {
  ...
  const rows = accData
    ...
    .map(d => `<tr> <td><strong>${d.age}</strong></td> <td>+${d.year}a</td> <td style="color:var(--text2)">${fmt(d.ral)}</td> <td style="color:var(--data)">${fmt(d.contrib)}</td> <td style="font-weight:600;color:var(--data)">${fmt(d.montanteINPS)}</td> <td style="font-weight:600;color:var(--purple)">${fmt(d.capFP)}</td> <td style="color:var(--text3)">${fmt(d.fpVersAnn)}</td> ${isNeg ? `<td style="color:var(--green)">${fmt(d.fpDatAnn)}</td>` : ''}
...
function renderPenDecTable(r) {
  ...
  const rows = decData
    ...
    .map(d => `<tr> <td><strong>${d.age}</strong></td> <td>+${d.year}a pens.</td> <td style="color:var(--text2)">${fmt(d.fabbisognoMens)}/m</td> <td style="color:var(--data);font-weight:600">${fmt(d.pensNettaMens)}/m</td> <td style="color:var(--purple)">${fmt(d.rendFPMens)}/m</td> <td style="color:var(--teal)">${fmt(d.etfMens)}/m</td> <td class="${d.gapMens===0?'pos':'neg'}">${d.gapMens===0?'Coperto':'−'+fmt(d.gapMens)+'/m'}</td> </tr>`).join('');
