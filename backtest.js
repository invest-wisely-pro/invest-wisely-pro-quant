...
function simulateBacktest(portKey, startYear, pacMonthly, w0, skipEvents, useCape) {
  ...
  // Rendimento bond del mese m: media pesata delle serie reali per scadenza + quota aggregata.
  // Ritorna null se non c'è mix reale (→ il chiamante usa row[1] come prima).
  function _bondRetReal(m) {
    if (!_bondMix || _obWtot <= 0) return null;
    let acc = 0;
    for (const key in _bondMix) {
      const w = _bondMix[key];
      if (key === '_agg') {
        acc += w * (_BOND_MEAN_M + _obDurK * (HIST_MONTHLY[m][1] - _BOND_MEAN_M));
      } else {
        const br = (typeof bondSeriesReturnAt === 'function') ? bondSeriesReturnAt(key, m) : null;
        acc += w * (br !== null ? br : HIST_MONTHLY[m][1]);
      }
    }
    return acc / _obWtot;
  }
